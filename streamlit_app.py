...
if 'iol' in S:
    if True:#try:
        ...
        with t_total:
            ...
            fig.add_trace(go.Scatter(x=vars_usd.index,y=vars_usd['SPY']*0,name='None',showlegend=False,marker_color='mediumspringgreen',mode='none',fill='tonexty',line=dict(dash='dashdot')))
            fig.add_hline(y=0,line_dash="dot",secondary_y=True,line_color="white",line_width=2)
            fig.update_layout(hovermode="x unified", margin=dict(l=1, r=1, t=25, b=1),height=450,bargap=0.2,legend=dict(
                                                orientation="h",
                                                yanchor="bottom",
